...
console.log(
  chalk.bold.italic.greenBright(
    "\t\t Code With SDA - Welcome to Student Management System "
  )
);
console.log(chalk.cyanBright("=".repeat(90)));
...
class Students {
  ...
  async addStudent() {
    ...
    console.log(
      chalk.bold.italic.greenBright(
        `Congratulations !!! ${chalk.bold.italic.blue(
          this.name
        )} is successfully enrolled in ${chalk.bold.italic.blue(this.courses)}.`
      )
    );
  }
...
while (condition) {
  ...
  //Show all students which enrolled
    ...
    if (indexToDelete !== -1) {
      ...
      console.log(
        chalk.bold.italic.greenBright(
          `Student "${chalk.bold.italic.blue(
      ...
      console.log(
        chalk.bold.italic.redBright(
          `Student "${chalk.bold.blue(
            deleteStudent.studentName
          )}"is not found in system's Database !!!`
        )
      );
    }
  }
  ...
  //Search Student
    ...
    for (const student of studentArray) {
      if (searchStudent.studentName == student.name) {
        ...
        console.log(
          chalk.bold.italic.gray("                  Courses: ") +
            chalk.bold.italic.greenBright(student.courses)
        );
        console.log(
          chalk.bold.italic.gray("                  Balance: ") +
    ...
    if (!found) {
      console.log(
        chalk.bold.italic.redBright(
          "Student Not found in system's Database !!!"
        )
      );
    }
  }
  ...
  //Fee Payment
        ...
        if (changeInNumber > student.balance) {
          ...
          console.log(
            chalk.bold.italic.greenBright(
              `Congratulations !!! ${chalk.bold.italic.blue(
                student.name
              )}'s fees ${chalk.bold.italic.blue(
                payFees.fees
              )} has been succesfully paid.`
            )
          );
        }
      } 
    ...
    async function markAttendance() {
      ...
      console.log(
        chalk.bold.italic.redBright(
          "Student not found or ID and name do not match !!!"
        )
      );
    }
